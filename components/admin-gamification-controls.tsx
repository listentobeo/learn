...
export function AdminGamificationControls({ enabled: initialEnabled, catalog: initialCatalog, students }: { enabled: boolean; catalog: CatalogRow[]; students: StudentRow[] }) {
  ...
  return <>
    <section className="surface game-master-switch"><div><span className="eyebrow">Master switch</span><h2>Student reward experience</h2><p>Pause new rewards and shop purchases without removing anyone’s earned progress or studio items.</p></div><button className={`game-toggle ${enabled ? "on" : ""}`} type="button" onClick={toggleGame} disabled={busy === "toggle"} aria-pressed={enabled}><span />{enabled ? "Active" : "Paused"}</button></section>
