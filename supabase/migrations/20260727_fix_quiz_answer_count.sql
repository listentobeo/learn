...
begin
  ...
  if jsonb_typeof(p_answers) <> 'object' then raise exception 'Invalid quiz answers'; end if;
  ...
  if not exists (
    ...
    where e.student_id = student
      ...
      and (
        l.track = 'Discovery'
        or l.week_number <= greatest(1, floor(extract(epoch from (now() - e.enrollment_date)) / 604800)::integer + 1)
      )
  ...
  if (select count(*) from jsonb_object_keys(p_answers)) <> total_questions or exists (
  ...
  return jsonb_build_object(
    'score', quiz_score,
    'attempt', attempt_no,
    'review', coalesce(answer_review, '[]'::jsonb)
  );
end;
$$;
